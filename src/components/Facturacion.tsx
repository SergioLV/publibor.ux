  ...
  return (
    <div className="fi-page">
      ...
      <div className="fi-mobile-cards">
          ...
          <div key={inv.id} className={`fi-mcard ${inv.status === 'failed' ? 'fi-mcard-failed' : ''}`} onClick={() => setDetailInvoice(inv)}>
            ...
            <div className="fi-mcard-bot">
              ...
              <div className="fi-mcard-actions">
                {inv.status === 'emitted' && (
                  <button className="fi-pdf-btn" onClick={(e) => handleViewPdf(inv.id, e)} disabled={loadingPdfId === inv.id} title="Ver PDF">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                  </button>
                )}
                {inv.status === 'failed' && (
        ...
        {!loading && filtered.length === 0 && (
          <div className="fi-mcard-empty">Sin documentos que mostrar</div>
        )}
      </div>
